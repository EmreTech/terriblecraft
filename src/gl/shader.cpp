...
void Shader::init(const std::string& vertexPath, const std::string& fragPath)
{
        ...
        unsigned int vertexShader = glCreateShader(GL_VERTEX_SHADER), fragShader = glCreateShader(GL_FRAGMENT_SHADER);
        const char *compileVertexCode = vertexCode.c_str(), *compileFragCode = fragCode.c_str();
...
void Shader::shaderSuccessfullyCompiled(unsigned int ID, std::string_view type)
{
    int success;
    char infoLog[512];
